...
//$servicios= $_POST['servicios'];
    if (isset($_POST['buscar'])){
        if (strlen($_POST['nombre_anuncio'])>=2)
            $select='SELECT * FROM `anuncio`,`ubicacion`';
            $where_nombre ="WHERE anuncio.id_anuncio = ubicacion.id_anuncio AND `nombre_anuncio` like '%".$_POST['nombre_anuncio']."%'";
      ...
      $sql_articulos = 'SELECT * FROM `anuncio`,`ubicacion` '.$where_nombre . $where_piscina1 . $where_hidro. $where_parq. $where_precio1. $where_precio2. $where_precio3. $where_ubicacion1. $where_ubicacion2. $where_ubicacion3. $where_ubicacion4. $where_join1. 'ORDER BY fecha desc LIMIT :iniciar,:narticulos ';

    }else {
      $sql_articulos = 'SELECT * FROM `anuncio` ORDER BY fecha desc LIMIT :iniciar,:narticulos ';
    }
